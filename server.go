...
*/
func HandleGET(path string, handler interface{}, headers map[string]string, object ...interface{}) {
	...
	httpHandler.writeResult = writeResultFunc(out, headers)
...
*/
func HandlePOST(path string, handler interface{}, headers map[string]string, object ...interface{}) {
	handlerFunc, in, out := getHandlerFunc(handler, object)
	httpHandler := &httpHandler{
		method:      "POST",
		handlerFunc: handlerFunc,
...
func setResponseHeaders(writer http.ResponseWriter, headers map[string]string) {
	for header := range headers {
		writer.Header().Set(header,  headers[header])
	}
}

func writeResultFunc(out []reflect.Type, headers map[string]string) func([]reflect.Value, http.ResponseWriter) {
